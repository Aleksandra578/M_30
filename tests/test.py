...
def test_show_all_pets(driver):
    ...
    # Вводим email
    driver.find_element(By.ID, 'email').send_keys('sdxc@yandex.ru')
    # Вводим пароль
    driver.find_element(By.ID, 'pass').send_keys('222024')
    # Нажимаем на кнопку входа в аккаунт
    driver.find_element(By.CSS_SELECTOR, 'button[type="submit"]').click()
    # Проверяем, что мы оказались на главной странице нужного пользователя
...
def test_no_duplicate_pets(driver):
    ...
    # Склеиваем имя, возраст и породу, получившиеся склеенные слова добавляем в строку и между ними вставляем пробел
    line = ''
    for i in list_data:
        line += ''.join(i)
        line += ' '
